...
class KernelTests(unittest.TestCase):
    ...
    def test01_average(self):
        ...
        request_parms['datainputs'] = [u'[domain=[{"id":"d0","level":{"start":0,"end":1,"system":"indices"}}];variable={"dset":"MERRA/mon/atmos","id":"v0:hur","domain":"d0"};operation=["CWT.average(v0,axis:xy)"];]']
    ...
    def test02_ensemble_average(self):
        ...
        request_parms = {'version': [u'1.0.0'], 'service': [u'WPS'], 'embedded': [u'true'], 'rawDataOutput': [u'result'], 'identifier': [u'cdas'], 'request': [u'Execute'] }
        request_parms['datainputs'] = [u'[domain=[{"id":"d0","level":{"start":0,"end":1,"system":"indices"}}];variable=[{"dset":"MERRA/mon/atmos","id":"v0:ta","domain":"d0"},{"dset":"CFSR/mon/atmos","id":"v0:ta","domain":"d0"}];operation=["CWT.average(*,axis:exy)"];]']
        response_json = taskManager.processRequest( request_parms )
        response = json.loads(response_json)
        response_data = response[0]['data']
        self.assertEqual( test_result, response_data[0:len(test_result)] )
