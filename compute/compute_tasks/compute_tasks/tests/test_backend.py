...
def test_error_handler(mocker):
    ...
    backend.error_handler(frames, state)
...
def test_resource_request(mocker):
    ...
    output = backend.resource_request(frames, env)
    ...
    env.get_template.assert_has_calls([
            ...
            mocker.call().render(data_claim_name='data-pvc', dev=False, image='aims2.llnl.gov/compute-celery', image_pull_secret=None, user='0', workers='10'),
            mocker.call().render(data_claim_name='data-pvc', dev=False, image='aims2.llnl.gov/compute-celery', image_pull_secret=None, user='0', workers='10'),
            mocker.call().render(data_claim_name='data-pvc', dev=False, image='aims2.llnl.gov/compute-celery', image_pull_secret=None, user='0', workers='10'),
    ])
...
def test_requeust_handler_exception(mocker):
    ...
    backend.request_handler(frames, state)
...
def test_build_workflow(mocker):
    ...
    workflow = backend.build_workflow(frames)

    job_started.s.assert_called_with('CDAT.subset', {'variable': [], 'domain': [], 'operation': []}, '0', '0', '0', {'DASK_SCHEDULER': 'dask-scheduler-0'})
...
def test_format_frames():
    ...
    output = backend.format_frames(frames)
