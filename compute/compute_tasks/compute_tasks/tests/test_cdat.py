...
def mpc():
    ...
    cert = m.logon(
        os.environ["MPC_USERNAME"], os.environ["MPC_PASSWORD"], bootstrap=True
    )
...
P0 = cwt.Process(
    ...
    inputs=V0,
    domain=cwt.Domain(time=("1979-01-01", "1979-06-01")),
)
...
def create_index(intervals, combine=True):
    indexes = [pd.Float64Index(pd.RangeIndex(x[0], x[1])) for x in intervals]
...
@pytest.mark.parametrize(
    ...
    [
        (((10, 40),), (20, 30), []),
        (((10, 40),), (0, 20), ((0, 10),)),
        (((10, 20), (30, 40)), (0, 50), ((0, 10), (20, 30), (40, 50))),
    ],
)
def test_find_missing_intervals(stored, requested, expected):
    stored_index = create_index(stored)

    requested_index = pd.Float64Index(
        pd.RangeIndex(requested[0], requested[1])
    )

    _, missing = cdat.find_missing_intervals(stored_index, requested_index)

    if expected is not None:
        expected = create_index(expected, False)

        for x, y in zip(missing, expected):
            assert np.all(x == y)
    else:
...
def test_build_workflow(mocker, input, expected):
    ...
    for x, y in zip(input, expected):
        ...
        assert (
            metrics.TASK_PROCESS_USED.labels(x.identifier)._value._value == 1
        )
        assert (
            metrics.TASK_PREPROCESS_BYTES.labels(x.identifier)._sum._value
            == y[0]
        )
        assert (
            metrics.TASK_POSTPROCESS_BYTES.labels(x.identifier)._sum._value
            == y[1]
        )
...
def test_groupby_bins_invalid_bin(test_data, mocker):
    ...
    ctx = context.OperationContext(
        0, 0, 0, 0, variable={}, domain={}, operation={}
    )
    ...
    with pytest.raises(WPSError):
        base.get_process(identifier)._process_func(
            ctx, p, *[v1], variable="pr", bins=["abcd"]
        )
...
def test_where_other(test_data, mocker):
    ...
    assert np.sum(result.pr).values == np.array(5.0156e24)
...
def params(**kwargs):
    p = {"variable": None, "fillna": None, "rename": None}
...
@pytest.mark.parametrize(
    "identifier,v1,v2,output,output_var,extra",
    [
        ...
        (
            ...
            np.array(1458000),
            "pr_test",
            params(const=4, rename=["pr", "pr_test"]),
        ),
        (
            "CDAT.divide",
            POS_5,
        ...
        (
            ...
            np.array(405000.0),
            "pr",
            params(const=None, variable=["pr", "pr"]),
        ),
        (
            "CDAT.divide",
            POS_5,
        ...
        (
            ...
            RAND,
            None,
            np.array(57172.20906099566),
            "pr",
            params(cond="pr>0.5", func="sum", other=None),
        ),
        (
            "CDAT.filter_map",
            RAND,
            ...
            np.array(1.11844e25),
            "pr",
            params(cond="pr>0.5", func="sum", other=1e20),
        ),
        (
            "CDAT.filter_map",
            RAND,
            ...
            np.array(57172.20906099566),
            "pr",
            params(cond="pr>0.5", func="sum", other=None, fillna=1e20),
        ),
        (
            "CDAT.filter_map",
            RAND,
        ...
        (
            ...
            np.array(4.285855641221728),
            "pr_test",
            params(axes=None, rename=["pr", "pr_test"]),
        ),
        (
            "CDAT.mean",
            RAND,
            None,
            np.array(0.005161633523396578),
            "pr",
            params(axes=None),
        ),
        (
...
def test_processing(
    mocker, test_data, identifier, v1, v2, output, output_var, extra
):
...
def test_process_aggregate(mocker, test_data):
    ...
    process = cwt.Process(
        identifier="CDAT.aggregate", inputs=[v0, v1, v2, v3]
    )
...
@pytest.mark.parametrize(
    "domain,decode_times,expected,params",
    [
        pytest.param(
            cwt.Domain(time=("1800-01", "1800-12")),
            True,
            (12, 64, 128),
            {},
...
def test_process_subset(
    test_data, mocker, domain, decode_times, expected, params
):
...
def test_build_split_output(test_data, mocker):
    ...
    delayed = cdat.build_split_output(
        ctx, ["pr"], data, p, "test", data.nbytes / 8
    )
...
@pytest.mark.parametrize(
    "identifier,inputs,domain,expected,expected_type",
    [
        ("CDAT.subset", [CMIP6_AGG1], None, 1, cftime.DatetimeNoLeap),
        (
            "CDAT.subtract",
            [CMIP6_AGG1, CMIP6_AGG2],
            None,
            2,
        ...
        (
            "CDAT.subset",
            [CMIP6_AGG1],
            cwt.Domain(time=(10, 200)),
            1,
            np.float64,
        ),
        (
            "CDAT.aggregate",
            [CMIP5_AGG1, CMIP5_AGG2],
            None,
            2,
            cftime.DatetimeNoLeap,
        ),
...
def test_gather_inputs(
    test_data, mocker, identifier, inputs, domain, expected, expected_type
):
    process = cwt.Process(identifier=identifier)
    process.add_inputs(
        *[cwt.Variable(test_data.local(x), "tas") for x in inputs]
    )
    process.set_domain(domain)
