...
def test_topo_sort_mixed_inputs(cwt_data):
    ...
    ctx = context.LocalContext.from_data_inputs(
        "CDAT.merge", workflow, user=0, process=0, status=0, job=0
    )
...
def test_topo_sort(cwt_data):
    ...
    ctx = context.LocalContext.from_data_inputs(
        "CDAT.workflow", workflow, user=0, process=0, status=0, job=0
    )
...
def test_interm_ops(cwt_data):
    ...
    assert len(ops) == 3
    assert set([x.identifier for x in ops]) == set(
        ["CDAT.aggregate", "CDAT.std", "CDAT.subtract"]
    )
...
def test_from_data_inputs_missing_operation():
    ...
    with pytest.raises(WPSError):
        context.LocalContext.from_data_inputs(
            "CDAT.subset", data_inputs, user=0, process=0, status=0, job=0
        )
...
def test_from_data_inputs_invalid_input_workflow(cwt_data):
    ...
    with pytest.raises(WPSError):
        context.LocalContext.from_data_inputs(
            "CDAT.workflow", data_inputs, user=0, process=0, status=0, job=0
        )
...
def test_from_data_inputs(cwt_data):
    ...
    ctx = context.LocalContext.from_data_inputs(
        "CDAT.aggregate", data_inputs, user=0, process=0, status=0, job=0
    )
...
def test_resolve_dependencies_globals(cwt_data):
    ...
    context.LocalContext.resolve_dependencies(
        variable, domain, operation_, cwt_data.d0, parameters
    )
...
def test_topo_sort(mocker):
    ...
    merge = cwt.Process(
        identifier="CDAT.merge", inputs=[s0.name, s1.name], name="merge"
    )
...
def test_generate_local_path_override_output(mocker):
    ...
    uuid4 = mocker.patch(
        "compute_tasks.context.uuid.uuid4", return_value="1234"
    )
    exists = mocker.patch(
        "compute_tasks.context.os.path.exists", return_value=False
    )
