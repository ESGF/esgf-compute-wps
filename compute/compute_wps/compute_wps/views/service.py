...
def int_or_float(value):
    ...
    try:
        ...
        raise exceptions.WPSError(
            'Failed to parse "{value}" as a float or int', value=value
        )
...
def get_parameter(params, name, required=True):
    ...
    if name.lower() not in temp:
        ...
        raise exceptions.WPSError(
            name.lower(), code=wps_response.MissingParameterValue
        )
...
def send_request_provisioner(
    identifier, data_inputs, job, user, process, status_id
):
    ...
    client.connect(
        "tcp://{!s}".format(settings.PROVISIONER_FRONTEND).encode()
    )
    ...
    payload = {
        ...
        "provenance": {
            ...
            "wps_document": utilities.data_inputs_to_document(
                identifier, data_inputs
            ),
        },
    ...
    try:
        ...
        raise exceptions.WPSError(
            "Error sending request to provisioner, retry in a few minutes."
        )
    ...
    try:
        ...
        raise exceptions.WPSError(
            "Error receiving acknowledgment from provisioner, retry in a few minutes."
        )
    else:
        if msg[0] == b"ACK":
            ...
            raise exceptions.WPSError(
                "Provisioner error, retry in a few minutes."
            )
        else:
            logger.info(
                "Provisioner responded with an unknown response %r", msg
            )
...
def handle_execute(request, identifier, data_inputs):
    ...
    if len(data_inputs_keys ^ REQUIRED_DATA_INPUTS) > 0:
        raise exceptions.WPSError(
            "{}",
            ", ".join(REQUIRED_DATA_INPUTS - data_inputs_keys),
            code=wps_response.MissingParameterValue,
        )
    ...
    elif settings.AUTH_TYPE == "noauth":
    ...
    try:
        ...
        raise exceptions.WPSError(
            'Process "{identifier}" does not exist', identifier=identifier
        )

    job = models.Job.objects.create(
        process=process, user=user, datainputs=json.dumps(data_inputs)
    )

    status_id = job.accepted()

    try:
        send_request_provisioner(
            identifier, data_inputs, job.id, user.id, process.id, status_id
        )
...
def handle_get(request):
    ...
    if param_request == "getcapabilities":
        ...
        raise exceptions.WPSError(
            'Operation "{name}" is not supported', name=param_request
        )
...
def handle_post(request):
    ...
    if "GetCapabilities" in doc.tag:
        raise exceptions.WPSError(
            "GetCapabilities POST request is not supported"
        )
    elif "DescribeProcess" in doc.tag:
        raise exceptions.WPSError(
            "DescribeProcess POST request is not supported"
        )
        ...
        try:
            ...
            raise exceptions.WPSError(
                "Invalid XML missing Identifier element"
            )
        ...
        for item in inputs:
            ...
            try:
                ...
                raise exceptions.WPSError(
                    "Invalid XML missing ComplexData element"
                )
...
def wps_entrypoint(request):
    ...
    try:
        ...
        response = wps_response.exception_report(
            wps_response.NoApplicableCode, error
        )
    except Exception as e:
        logger.exception(
            "Some generic exception %r %r", request.method, request.path
        )

        error = "Please copy the error and report on Github: {}".format(
            str(e)
        )
