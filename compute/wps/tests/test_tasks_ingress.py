...
class PreprocessTestCase(test.TestCase):
    ...
    @mock.patch('wps.models.Cache')
    def test_ingress_cache_multiple_inputs(self, mock_create, mock_open, mock_cache):
        mock_cache.return_value = None

        self.ingress1.update(self.ingress2)

        data = ingress.ingress_cache(self.ingress1, *self.args2)
    ...
    @mock.patch('wps.tasks.preprocess.check_cache_entries')
    @mock.patch('wps.tasks.base.CWTBaseTask.open')
    @mock.patch('wps.models.Cache')
    ...
    @mock.patch('wps.tasks.base.CWTBaseTask.load_credentials')
    ...
    @mock.patch('wps.tasks.ingress.read_data')
    @mock.patch('os.stat')
    def test_ingress_uri(self, mock_stat, mock_read, mock_open, mock_now,
                         mock_cred, mock_job):
        type(mock_stat.return_value).st_size = 1200000
